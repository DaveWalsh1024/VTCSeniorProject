...
public class HomeTeamScorecardActivity extends AppCompatActivity {
    ...
    public void fillTable() {
        ...
        for (batter = 1; batter <= 9; batter++) {
            ...
            tv.setText(game.getHomeTeamBattingOrder().get(batter-1).getLastName());
        ...
        for (int k = 0; k < game.getPlays().size(); k++)
        {
            ...
            if (game.getPlays().get(k).getTopOrBottom() == 2)
            {
                ...
                {
                    re.updateScorecardBox(findScoreCardBox(re.getCurrentInning(), re.getRunnerBattingOrderPosition() - 1));
                }
